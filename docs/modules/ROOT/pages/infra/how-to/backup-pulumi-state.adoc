...
| Create a `Pulumi backups` folder; upload export JSON after apply. Already used for founder Drive (`rj.amdphreak@gmail.com`).

| **`rclone`** (if installed)
| `rclone copy ./pulumi-state-*.json gdrive:Pulumi-backups/` — good for scheduled jobs.

| **DevCentr RepoDrive** (`repodrive`)
| **Not** Google Drive. Mounts forge repos as a virtual filesystem. Wrong tool for Pulumi checkpoints.
|===
